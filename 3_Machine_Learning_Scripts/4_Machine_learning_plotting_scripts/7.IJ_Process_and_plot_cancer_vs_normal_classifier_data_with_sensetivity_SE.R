...
get_labels <- function(unique_metrics) {
  label_mapping <- list(
    "Combined_all_features_1pct_validation" = "Combined All Features (1%)",
    "Combined_all_features_5pct_validation" = "Combined All Features (5%)",
    "Combined_motif_methylation_PCs_1pct_validation" = "Combined Motif + Methylation (1%)",
    "Combined_motif_methylation_ratios_PCs_1pct_validation" = "Combined Motif + Methylation + Ratios (1%)",
    "Combined_ratios_methylation_PCs_1pct_validation" = "Combined Ratios + Methylation (1%)",
    "End_motifs_validation" = "End Motifs",
    "Insert_size_validation" = "Insert Size",
    "Methylation_validation" = "Methylation",
    "Nucleosome_peak_validation" = "Nucleosome Peak",
    "Fragment_ratios_validation" = "Fragment Ratio",   
...
for (i in 1:nrow(unique_comparisons_df)) {
  ...
  for (current_cohort in unique(subset_class_df$Cohort)) {
    ...
    sens99_points_all <- tibble()   # <= add this line here (new)
    ...
    for (j in seq_along(filenames)) {
      ...
      auc_per_fold <- data %>% group_by(Fold) %>%
        summarise(
          auc = if (length(unique(ActualClass)) < 2) NA_real_ else {
            roc_obj <- roc(
              response = ActualClass,
              predictor = prob_cancer,
              levels = c(type2, type1),  # here type2 (normal) is the negative and type1 (cancer) is positive
              direction = "<"            # higher prob_cancer indicates cancer
            )
      ...
      # keep per-fold values for plotting
      sens99_points_all <- dplyr::bind_rows(
        sens99_points_all,
        tibble(
          Metric = analysis,
          sens99 = as.numeric(sens_at_99_per_fold),
          Fold   = names(sens_at_99_per_fold),  # may be NA; harmless
          File   = filename_short,
          Cohort = current_cohort
        )
      )
    ...
    sens99_points_all$Metric <- factor(
      sens99_points_all$Metric,
      levels = unique_metrics,
      labels = get_labels(unique_metrics)
    )
    ...
    # ---- NEW: Export the detailed ROC/AUC/CM/Sens99 data (across folds)
    write.csv(data_auc, file.path(path, paste0("detailed_auc_", type1, "_vs_", type2, "_", current_cohort, ".csv")), row.names = FALSE)
    write.csv(data_roc, file.path(path, paste0("detailed_roc_", type1, "_vs_", type2, "_", current_cohort, ".csv")), row.names = FALSE)
    write.csv(data_cm, file.path(path, paste0("detailed_cm_", type1, "_vs_", type2, "_", current_cohort, ".csv")), row.names = FALSE)
    write.csv(data_sens99, file.path(path, paste0("detailed_sens99_", type1, "_vs_", type2, "_", current_cohort, ".csv")), row.names = FALSE)
...
for (j in seq_along(filenames)) {
 ...
 # data$ActualClass_numeric <- ifelse(data$ActualClass == type2, 0, 1)
  ...
  roc_list <- lapply(unique(data$Fold), function(fold) {
    fold_data <- data %>% filter(Fold == fold)
    if (length(unique(fold_data$ActualClass)) < 2) return(NULL)
    # Force the factor levels so that type2 (healthy) is negative and type1 (cancer) is positive
    roc(fold_data$ActualClass, fold_data$prob_cancer, levels = c(type2, type1), direction = "<")
  })
  ...
  sens_at_99_per_fold <- sapply(roc_list, function(roc_obj) {
    # Try to extract sensitivity at 99% specificity; if error, return NA
    val <- tryCatch({
      as.numeric(coords(roc_obj, x = 0.99, input = "specificity", ret = "sensitivity", transpose = FALSE))[1]
    }, error = function(e) { NA })
    # If no valid coordinate is found, set sensitivity to 0

# ==============================================================================
# Purpose: 4 1 Process and plot nucelosome peak distances
...
# How to run: Rscript 2_Fragmentomics_Scripts/R_scripts_to_process_features/1_Fragmentomic_feature_processing_and_plotting_scripts/4.1_Process_and_plot_nucelosome_peak_distances.R
...
# ==============================================================================
# ----------------------------------------------------------------------------
# Title   : Nucleosome Peak Distance Analysis – Profiles & Comparative Statistics
...
#
# Purpose :
#   1.   Import nucleosome peak distance histograms for healthy & cancer samples
#   2.   Aggregate, normalize (to %), and restrict distance windows (±300 bp)
#   3.   Merge with sample metadata and filter to paired‐end cfDNA
#   4.   Compute healthy median + SD, per‐cancer medians, z‐scores & KS‐test p‑values
#   5.   Generate line‐plots of frequency, difference, and z‑score by cancer type
#   6.   Perform pairwise KS‐tests and plot delta distributions for all comparisons
#   7.   Conduct PCA on distance matrices; export variance plots & PC scores
#   8.   Save processed matrices, z‑score tables, and diagnostic figures for publication & ML
#
# ----------------------------------------------------------------------------
...
## Now exclude validation and resplit 
data_sum <- data_sum %>%
  dplyr::filter(
    sample %in% metadata_df$nucleosome_peak_sampleid & 
      metadata_df$validation[match(sample, metadata_df$nucleosome_peak_sampleid)] == 0 &
...
cancer_types <- c("brain_cancer", "head_and_neck_cancer", "AML", "prostate_cancer",
                  "lung_cancer", "lfs_survivor", "lfs_previvor", "lfs_positive", "eye_cancer", "healthy") 
...
data_plot$cancer_status <- factor(data_plot$cancer_status, levels = c("brain_cancer", "head_and_neck_cancer", "AML", "prostate_cancer", "lung_cancer", "lfs_survivor", "lfs_previvor", "lfs_positive", "eye_cancer", "healthy"))

### Set hlines
data_lines <- data.frame(cancer_status = rep(c("brain_cancer", "head_and_neck_cancer", "AML", "prostate_cancer", "lung_cancer", "lfs_survivor", "lfs_previvor", "lfs_positive", "eye_cancer", "healthy"), 2),
...
data_lines$cancer_status <- factor(data_lines$cancer_status, levels = c("brain_cancer", "head_and_neck_cancer", "AML", "prostate_cancer", "lung_cancer", "lfs_survivor", "lfs_previvor", "lfs_positive", "eye_cancer", "healthy"))
...
#data_plot_filtered_reordered$cancer_type_title_case  <- gsub("Eye_cancer", "Uveal_melanoma", data_plot_filtered_reordered$cancer_type_title_case)
...
fig <- fig_nuc_peaks
ggsave(file.path(outdir, paste0("nucleosome_peaks_167bp cfMeDIP with pvalue, updated normals, Mar 2025, updated no pvalue, 3.pdf")), fig, width = 15, height = 7, units = "in", dpi = 500)
ggsave(file.path(outdir, paste0("nucleosome_peaks_167bp cfMeDIP with pvalue, updated normals, Mar 2025, updated no pvalue, 4.pdf")), fig, width = 15, height = 7*1.15, units = "in", dpi = 500)
saveRDS(data_plot_filtered_reordered, file = "Dataframe for nucleosome peaks figure Mar 2025.rds")

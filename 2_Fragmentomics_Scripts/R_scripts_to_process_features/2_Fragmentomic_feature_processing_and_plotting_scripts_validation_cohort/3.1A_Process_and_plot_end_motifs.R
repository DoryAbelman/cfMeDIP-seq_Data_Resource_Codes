# ==============================================================================
# Purpose: 3 1A Process and plot end motifs
...
# How to run: Rscript 2_Fragmentomics_Scripts/R_scripts_to_process_features/2_Fragmentomic_feature_processing_and_plotting_scripts_validation_cohort/3.1A_Process_and_plot_end_motifs.R
...
### Now keep the SE and validation for time being 
data_endmotif_filtered_SE <- data_endmotif_filtered %>% filter(type == "SE")
data_endmotif_filtered_validation <- data_endmotif_filtered %>% 
  filter(validation == "1", 
         type == "PE", 
         cancer_type_title_case != "Liver_cancer")
#data_endmotif_filtered <- data_endmotif_filtered %>% filter(validation == "0") %>% filter(type == "PE")


### First do the validation 
data_endmotif_filtered <- data_endmotif_filtered_validation
...
fig <- ggplot(data_dnase) +
  ...
  geom_text(data = data_stats_dnase, aes(cancer_type_title_case, 3.2, label = annot), size = 5) +
  xlab("Frequency (%)") + 
  ylab("Frequency (%)") +
  labs(color = "", fill = "") +
...
ggsave(file.path(outdir, paste0("fragment_end_contexts_dnase1l3, March 2025, updated to all frags 5prime only, 3, validation.pdf")), fig, width = 15, height = 8, dpi = 500, units = "in")
...
ggsave(file.path(outdir, paste("fragment_fold_change_all_cancers, March 2025, ordered by prop short insert size, all frags, narrower.pdf", sep = "")), fig_fold_cancer_ordered, width = 10, height = 3, dpi = 500, units = "in")
...
for(cancer_type in unique_cancer_types){
  ...
  ggsave(file.path(outdir, paste("fragment_end_contexts_change_", cancer_type, "March 2025.pdf", sep = "")), figure, width = 10, height = 6, dpi = 500, units = "in")
}
...
for(cancer_type in unique_cancer_types){
 ...
 # data_motif[data_motif == "G" | data_motif == "C"] <- "G/C"
  ...
  # Save the combined plot
  ggsave(file.path(outdir, paste("fragment_end_contexts_change_different_colors_", cancer_type, "March 2025.pdf", sep = "")), figure, width = 10, height = 6, dpi = 500, units = "in")
}
...
saveRDS(metadata_df, file = "Metadata df for end motifs, May 2024 validation.rds")
...
saveRDS(data_stats, file = file.path(outdir, "motif_statistics_by_cancer_type_validation.rds"))
saveRDS(data_stats_dnase, file = file.path(outdir, "DNASE1L3_motif_statistics_by_cancer_type_validation.rds"))
saveRDS(fold_change_all, file = file.path(outdir, "fold_change_all_cancer_vs_normal_validation.rds"))
saveRDS(data_reg, file = file.path(outdir, "regression_data_cancer_comparisons_validation.rds"))
saveRDS(final_shannon, file = file.path(outdir, "shannon_index_statistics_by_cancer_type_validation.rds"))
saveRDS(final_shannon2, file = file.path(outdir, "detailed_shannon_statistics_AT_content_by_cancer_type_validation.rds"))


write.csv(data_stats_dnase, 
          file = file.path(outdir, "DNASE1L3_motif_statistics_by_cancer_type_validation.csv"), 
          row.names = FALSE)
...
### Save objects
saveRDS(Normal_ratio, file=file.path(outdir, paste0("Normal_ratio_Endmotif_validation.rds")))
saveRDS(Cancer_ratio, file=file.path(outdir, paste0("Cancer_ratio_Endmotif_validation.rds")))
saveRDS(Normal_median, file=file.path(outdir, paste0("Normal_median_Endmotif_validation.rds")))
saveRDS(zscores_mat_endmotif, file = file.path(outdir, paste0("Zscore_mat_Endmotif_validation.rds")))
saveRDS(End_motif_df_zscores, file = "Zscores df for end motifs_validation.rds")
...
png(filename = "Heatmap of endmotif by cancer type, updated legends, updated colors, with breaks, Mar 2025, all frags 5' ends only, 20 to 600bp.png", width = 20, height = 25, units = "in", res = 350)
...
### Cap 
# Cap any values >45 to 45 (and handle Inf values)
zscores_ordered_endmotif_capped <- pmin(zscores_ordered_endmotif, 20)
zscores_ordered_endmotif_capped[is.infinite(zscores_ordered_endmotif_capped)] <- 20

right_annotation <- rowAnnotation(
  "Genome-Wide\nZ-score" = anno_lines(
    zscores_ordered_endmotif_capped,
    add_points = TRUE,
    pch = c(16, NA),
    ylim = c(-2, 25),   # extend the y-limit to 45
    gp = gpar(col = c("black", "red"),
              lty = c("solid", "dashed")),
    axis_param = list(
      side = "bottom",
      at = c(-2, 0, 10, 20),  # tick positions
      labels = c("", "0", "10", ">20"),  # replace 25 with ">25"
      labels_rot = 0,
      gp = gpar(fontsize = 10)
    )
  ),
...
png(filename = "Heatmap of end motif proportions by cancer type, updated legends, updated colors, with breaks, split, Mar 2025, updated 5' only all frags, 20-600bp.png", width = 18, height = 25, units = "in", res = 350)
draw(Heatmap_endmotif_freq, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()

png(filename = "Heatmap of end motif proportions by cancer type, updated legends, updated colors, with breaks, split, Mar 2025, updated 5' only all frags, 20-600bp, 2.png", width = 13, height = 17, units = "in", res = 500)
draw(Heatmap_endmotif_freq, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()
...
png(filename = "Heatmap of end motif proportions by cancer type, updated legends, updated colors, with breaks, split, Mar 2025, updated 5' only all frags, 20-600bp, zscores, updated color.png", width = 18, height = 25, units = "in", res = 350)
draw(Heatmap_endmotif_freq_zscore, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()

png(filename = "Heatmap of end motif proportions by cancer type, updated legends, updated colors, with breaks, split, Mar 2025, updated 5' only all frags, 20-600bp, zscores, updated color, narrow 2.png", width = 18, height = 22, units = "in", res = 350)
draw(Heatmap_endmotif_freq_zscore, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()
...
saveRDS(endmotif_matrix, file=file.path(outdir, paste0("End_motif_matrix_June2024_validation.rds")))
saveRDS(metadata_df, file=file.path(outdir, paste0("Metadata_df_endmotif_June2024_validation.rds")))
...
# Print the plot
print(p)
ggsave(filename = file.path(outdir, "Cumulative Explained Variance by Principal Components end motifs validation.png"), plot = p, width = 7, height = 5, units = "in", dpi = 300)
...
ggsave(filename = file.path(outdir, "PCA_Variance_Explained_Endmotif_validation.png"), plot = variance_plot, width = 10, height = 6, units = "in", dpi = 300)

# Save the PCA results to a file 
write.csv(pca_df_endmotif, file.path(outdir, "PCA_results_End_motif validation.csv"), row.names = TRUE)
saveRDS(pca_df_endmotif, file = "End_motifs_updated_June2024_PCAs_97_percent_variance_validation.rds")
saveRDS(pca_result_endmotif, file = "PCA results for end motifs_validation.rds")
...
############## 
### Now repeat the analysis for SE data 

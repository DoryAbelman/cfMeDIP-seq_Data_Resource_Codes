...
# Original date   : 2023‑08‑16
# Updates:
#   • Oct–Nov 2023      – added new fragment datasets
#   • May 2024          – finalized prostate cancer revisions
#   • Dec 2024–Apr 2025 – manuscript resubmission edits
#
# Publication Reference:
#   Integrated, Longitudinal Analysis of Cell‑free DNA in Uveal Melanoma
#   https://github.com/pughlab/TGL48_Uveal_Melanoma/blob/main/R_scripts_figures/Figure%202/Figure%202%20-%20Fragment%20Ratio.R
#   PMC9973415
#
# Purpose :
#   1.   Import and merge 5′ end‐motif frequency tables for healthy & cancer samples
#   2.   Normalize frequencies and filter low‐quality/ambiguous motifs
#   3.   Compute motif‐wise statistics (mean, SD, fold‐change, adjusted p‑values)
#   4.   Derive DNASE1L3‐associated motif scores and overall fold‑change matrices
#   5.   Generate boxplots, heatmaps, regression comparisons, and Shannon index charts
#   6.   Calculate genome‑wide z‑scores of end‑motif deviations vs. healthy controls
#   7.   Export processed RDS/CSV matrices and figures for downstream analysis & ML
#
# ----------------------------------------------------------------------------
...
cat("Number of samples after merging with metadata:", length(unique(data_endmotif$sample)), "\n")
...
cat("Number of samples after filtering for non-NA cancer_type, validation==0, and type=='PE':", length(unique(data_endmotif_filtered$sample)), "\n")
...
#### Get fold change per sample 
# 1) Identify DNASE1L3-associated motifs
data_dnase <- data_endmotif_filtered %>%
  filter(motif %in% c("CCCA", "CCAG", "CCTG", "CCAA", "CCCT", "CCAT"))

# 2) Calculate the average normalized frequency for healthy controls, by motif
healthy_means <- data_dnase %>%
  filter(cancer_type_corrected_updated == "healthy") %>%
  group_by(motif) %>%
  summarize(mean_healthy = mean(normalized_frequency, na.rm = TRUE)) %>%
  ungroup()

# 3) Merge these healthy means back into data_dnase and compute per-sample fold change
data_dnase_fc <- data_dnase %>%
  left_join(healthy_means, by = "motif") %>%
  mutate(fold_change_sample = normalized_frequency / mean_healthy)

# data_dnase_fc now has a fold_change_sample column for each row (sample × motif)
# This fold_change_sample is the ratio of that sample's motif frequency to the healthy average.

## Get score 
dnase1l3_score <- data_dnase_fc %>%
  group_by(sample) %>%
  summarize(
    DNASE1L3_mean_score = mean(fold_change_sample, na.rm = TRUE),
    DNASE1L3_median_score = median(fold_change_sample, na.rm = TRUE),
    DNASE1L3_sd_score = sd(fold_change_sample, na.rm = TRUE),
    motif_count = n()
  ) %>%
  ungroup()

# Export
write.csv(dnase1l3_score, file = "DNASE1L3_score_PE.csv", row.names = FALSE)
...
data_endmotif_filtered$cancer_type_corrected_updated <- recode(
  data_endmotif_filtered$cancer_type_corrected_updated, 
  "aml" = "AML"
)
# Loop through each unique cancer type in data_endmotif_filtered
unique_cancer_types <- c("brain_cancer", "head_and_neck_cancer", "AML", "prostate_cancer", "lung_cancer", "lfs_survivor", "lfs_previvor", "lfs_positive", "eye_cancer", "healthy")
...
#  data_cancer <- data_endmotif_filtered %>% filter(cancer_type_corrected_updated == "AML")
...
fig <- ggplot(data_dnase) +
  ...
  geom_text(data = data_stats_dnase, aes(cancer_type_title_case, 4, label = annot), size = 5) +
  xlab("Frequency (%)") + 
  ylab("Frequency (%)") +
  labs(color = "", fill = "") +
...
fig_fold_cancer_ordered <- ggplot(fold_change_all, aes(x = cancer_type_ordered, y = fold_change, col = cancer_type_ordered)) +
  ...
  ylab("Fold Change") +
  scale_color_manual(values = cancer_type_col_split) +
  scale_x_discrete(labels = function(x) ifelse(x == "Eye_cancer", "Uveal_Melanoma", x)) +
  ggtitle("Fold change of cancers vs normal samples, by motif") + 
  theme +
  theme(legend.position = "none",
        axis.ticks.x = element_blank(),
...
ggsave(file.path(outdir, paste("fragment_fold_change_all_cancers, May 2024, ordered by prop short insert size, all frags, narrower, updated 3.pdf", sep = "")), fig_fold_cancer_ordered, width = 10, height = 3, dpi = 500, units = "in")
...
saveRDS(data_motif_wide, file = "Dataframe matrix for end motifs, all frags, May 2024.rds")
saveRDS(metadata_df, file = "Metadata df for end motifs, May 2024.rds")
...
saveRDS(data_stats, file = file.path(outdir, "motif_statistics_by_cancer_type.rds"))
saveRDS(data_stats_dnase, file = file.path(outdir, "DNASE1L3_motif_statistics_by_cancer_type.rds"))
saveRDS(fold_change_all, file = file.path(outdir, "fold_change_all_cancer_vs_normal.rds"))
saveRDS(data_reg, file = file.path(outdir, "regression_data_cancer_comparisons.rds"))
saveRDS(final_shannon, file = file.path(outdir, "shannon_index_statistics_by_cancer_type.rds"))
...
### Save objects
saveRDS(Normal_ratio, file=file.path(outdir, paste0("Normal_ratio_Endmotif.rds")))
saveRDS(Cancer_ratio, file=file.path(outdir, paste0("Cancer_ratio_Endmotif.rds")))
saveRDS(Normal_median, file=file.path(outdir, paste0("Normal_median_Endmotif.rds")))
saveRDS(zscores_mat_endmotif, file = file.path(outdir, paste0("Zscore_mat_Endmotif.rds")))
saveRDS(End_motif_df_zscores, file = "Zscores df for end motifs.rds")
...
row_split <- data_cancer_type  # now a vector
...
png(filename = "Heatmap of endmotif by cancer type, updated legends, updated colors, with breaks, May 2024, all frags 5' ends only, 20 to 600bp.png", width = 20, height = 25, units = "in", res = 350)
...
saveRDS(endmotif_matrix, file=file.path(outdir, paste0("End_motif_matrix_Mar2025.rds")))
saveRDS(metadata_df, file=file.path(outdir, paste0("Metadata_df_endmotif_Mar2025.rds")))
...
# Print the plot
print(p)
ggsave(filename = file.path(outdir, "Cumulative Explained Variance by Principal Components end motifs.png"), plot = p, width = 7, height = 5, units = "in", dpi = 300)
...
variance_plot <- fviz_eig(pca_result_endmotif, addlabels = TRUE, ylim = c(0, 30)) +
  theme_classic() +
...
ggsave(filename = file.path(outdir, "PCA_Variance_Explained_Endmotif.png"), plot = variance_plot, width = 10, height = 6, units = "in", dpi = 300)

# Save the PCA results to a file 
write.csv(pca_df_endmotif, file.path(outdir, "PCA_results_End_motif.csv"), row.names = TRUE)
saveRDS(pca_df_endmotif, file = "End_motifs_updated_June2024_PCAs_97_percent_variance.rds")
saveRDS(pca_result_endmotif, file = "PCA results for end motifs.rds")
...
data_cancer_type <- factor(metadata_df$cancer_type_title_case, levels = c("Healthy", "Prostate_cancer", "Head_and_neck_cancer", "Brain_cancer", "AML", "Eye_cancer", "LFS_survivor", "LFS_previvor", "LFS_positive", "Lung_cancer"))
...
write.csv(data_stats_dnase, file = file.path(outdir, "DNASE1L3_statistics_PE_Endmotif.csv"), row.names = FALSE)

write.csv(data_dnase, file = file.path(outdir, "DNASE1L3_all_fold_changes_PE.csv"), row.names = FALSE)
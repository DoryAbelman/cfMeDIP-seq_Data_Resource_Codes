###############################################################################
# File:      10.2A_Process_Yuanchang_Data_Cancer_Subtype_Classification_updated
# Author:    Dory Abelman
# Date:      2025‑03‑30
#
# Purpose:
...
path <- "/Users/dabelman/Documents/Thesis_work/R/M4/Projects/Fragmentation_CfMeDIP_Yong/Final Data Dec 2024/Yuanchang_ML_results/cfMEDIP_clf_res_20250330/results_subtypes/"
...
# Join
sum_data$cancer <- sum_data$subtype # for ease of naming
sum_data$cancer <- str_to_title(sum_data$cancer)  # converts subtype values to title case

# Replace beginning "Idh" with "IDH" (case-insensitive)
sum_data$cancer <- str_replace_all(sum_data$cancer, regex("^Idh", ignore_case = TRUE), "IDH")

# Replace beginning "Lfs" with "LFS" (case-insensitive)
sum_data$cancer <- str_replace_all(sum_data$cancer, regex("^Lfs", ignore_case = TRUE), "LFS")

sum_data <- sum_data %>%
  mutate(cancer = if_else(cancer == "Eye Cancer", "Uveal Melanoma", cancer))


# Apply recoding for cancers with exactly two subtypes:
sum_data <- sum_data %>%
  mutate(cancer = case_when(
    # Merge only if there are exactly two subtypes
    cancer %in% c("IDH Mutant Glioma", "IDH Wildtype Glioma") ~ "IDH Gliomas (Mut vs WT)",
    tolower(cancer) %in% c("metastatic castration resistant prostate cancer", "primary prostate cancer") ~ "Prostate Cancer (Primary vs mCRPC)",
    # Otherwise, keep the original cancer label (e.g., for Brain Cancer or LFS with >2 classes)
    TRUE ~ cancer
  ))
...
write_csv(best_feature_summary, "best_feature_summary_subtype.csv")
write_csv(best_model_summary, "best_model_summary_subtype.csv")
write_csv(fold_summary, "fold_summary_subtype.csv")
write_csv(best_model_by_cancer, "best_model_by_cancer_subtype_subtype.csv")
...
ggsave("Mean_ROC_curves_by_cancer_with_CI_best_model_subtype.png", roc_plot, width = 10, height = 8, dpi = 300)
print(roc_plot)
...
### ---------------------------
# Here we assume sum_data (from your .sum_subtype.csv files) has a column named
...
## Rename 
tmp <- roc_avg_dt %>%
  mutate(cancer = if_else(cancer == "Metastatic Castration Resistant Prostate Cancer", 
                          "Met. C.R. Prostate Cancer", 
                          cancer))

roc_plot <- ggplot(tmp, aes(x = fpr, y = tpr, color = feature_label, fill = feature_label)) +
  ...
  facet_wrap(~ cancer, scales = "free", labeller = label_wrap_gen(width = 29)) +
...
ggsave("Mean_ROC_curves_by_cancer_with_CI_custom_best_model_subtype_thick_label.png", roc_plot, width = 10, height = 8, dpi = 500)
print(roc_plot)
...
ggsave("CM_bar_plot_best_model_subtype.png", CM_bar_plot, width = 12, height = 16, dpi = 500)
...
print(ClevelandPlot)
ggsave("ClevelandPlot_Sensitivity95_wide_best_model_subtype.png", ClevelandPlot, width = 14, height = 10, dpi = 500)
ggsave("ClevelandPlot_Sensitivity95_narrow_best_model_subtype.png", ClevelandPlot, width = 7, height = 10, dpi = 500)
ggsave("ClevelandPlot_Sensitivity95_narrow_best_model2_subtype.png", ClevelandPlot, width = 7, height = 13, dpi = 500)
...
# Print and save the plot
print(ClevelandPlot)
ggsave("ClevelandPlot_Sensitivity95_LargerText_narrow_subtype.png", ClevelandPlot, width = 7, height = 10, dpi = 500)
...
ggsave("Combined_ROC_and_CM_custom_subtype.png", final_fig, width = 12, height = 6, dpi = 300)
print(final_fig)
...
ggsave("ClevelandPlot_Models_AUC_subtype.png", cleveland_plot, width = 10, height = 8, dpi = 300)
...
# Save and print the Cleveland plot
ggsave("ClevelandPlot_Models_AUC_updated_subtype.png", cleveland_plot, width = 13, height = 10, dpi = 500)
ggsave("ClevelandPlot_Models_AUC_updated_subtype_smaller.png", cleveland_plot, width = 13, height = 8, dpi = 500)
print(cleveland_plot)

ggsave("ClevelandPlot_Models_AUC_updated_subtype_long.png", cleveland_plot, width = 6, height = 10, dpi = 500)
ggsave("ClevelandPlot_Models_AUC_updated_subtype_long2.png", cleveland_plot, width = 7, height = 10, dpi = 500)
...
export_dir <- "Final Data Dec 2024/ML_Data/Yuanchang_processed_cancer_subtype_updated"
...
#best_feature_summary <- read.csv("Final Data Dec 2024/ML_data/Yuanchang_processed_cancer_subtype/best_feature_summary_subtype.csv")
...
write_csv(best_feature_summary, file.path(export_dir, "best_feature_summary_subtype.csv"))
write_csv(top_feature_per_cancer, file.path(export_dir, "top_feature_per_cancer_subtype.csv"))
...
write_csv(best_model_summary, file.path(export_dir, "best_model_summary_subtype.csv"))

# 3. Fold-level Summary Data
write_csv(fold_summary, file.path(export_dir, "fold_summary_subtype.csv"))

# 4. Best Model by Cancer (the chosen model per cancer type)
write_csv(best_model_by_cancer, file.path(export_dir, "best_model_by_cancer_subtype.csv"))

# 5. Performance Summary (based on best model data; by cancer and feature)
write_csv(performance_summary, file.path(export_dir, "performance_summary_subtype.csv"))

# 6. Overall Feature Performance (averaged across cancers)
write_csv(overall_feature_performance, file.path(export_dir, "overall_feature_performance_subtype.csv"))

# 7. Methylation-only Summary
write_csv(methyl_summary, file.path(export_dir, "methyl_summary_subtype.csv"))

# 8. Combined Features Summary (general, i.e. features containing "all" or "+")
write_csv(combined_summary, file.path(export_dir, "combined_summary_subtype.csv"))

# 9. Comparison of Methylation-only vs. Combined Models
write_csv(comparison, file.path(export_dir, "comparison_methyl_vs_combined_subtype.csv"))

# 10. Comparison of Methylation-only vs. 'motif+methyl+delfi'
write_csv(comparison_specific, file.path(export_dir, "comparison_methyl_vs_motif+methyl+delfi_subtype.csv"))

# 11. Overall Gains (Combined vs. Methylation-only)
write_csv(overall_combined, file.path(export_dir, "overall_gains_combined_vs_methyl_subtype.csv"))

# 12. Overall Gains (motif+methyl+delfi vs. Methylation-only)
write_csv(overall_specific, file.path(export_dir, "overall_gains_motif+methyl+delfi_vs_methyl_subtype.csv"))

# 13. Model Summary (by cancer and model; before mapping to longer names)
write_csv(model_summary, file.path(export_dir, "model_summary_subtype.csv"))

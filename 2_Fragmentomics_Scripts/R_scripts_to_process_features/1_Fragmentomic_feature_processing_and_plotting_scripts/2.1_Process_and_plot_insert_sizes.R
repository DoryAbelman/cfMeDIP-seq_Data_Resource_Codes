# ==============================================================================
# Purpose: 2 1 Process and plot insert sizes
...
# How to run: Rscript 2_Fragmentomics_Scripts/R_scripts_to_process_features/1_Fragmentomic_feature_processing_and_plotting_scripts/2.1_Process_and_plot_insert_sizes.R
...
# Updates:
#   • Oct–Nov 2023      – added new sample files
#   • May 2024          – updated prostate cancer sample processing
#   • Dec 2024–Apr 2025 – revisions for manuscript resubmission

# Purpose :
#   1.   Load insert‑size frequency files from cfMeDIP‑seq outputs
#   2.   Merge and reshape raw counts into per‑sample fragment length proportions
#   3.   Compute healthy median, SD, and z‑score deviations for cancer vs. normal
#   4.   Generate heatmaps, PCA plots, and boxplots of insert‑size distributions (PCA for reference only and not used in ML - that is generated in later scripts (see ML section))
#   5.   Export processed matrices and statistical results for downstream analysis
...
insert_size_df_validation <- insert_size_df %>% filter(validation == "1")
insert_size_df <- insert_size_df %>% filter(validation == "0")
healthy_insert <- insert_size_df %>% dplyr::filter(cancer_type_corrected_updated == "healthy") %>% 
...
cancer_insert <- insert_size_df %>% dplyr::filter(cancer_type_corrected_updated != "healthy") %>% 
...
Insert_df_zscores <- zscore_df 
Insert_df_zscores <- rownames_to_column(Insert_df_zscores, "Sample")

# Join with metadata df
Insert_df_zscores <- left_join(Insert_df_zscores, metadata_df, by = c("Sample" = "sample_id"))

### Save objects
saveRDS(Normal_ratio, file=file.path(outdir, paste0("Normal_ratio_Insert_size.rds")))
saveRDS(Cancer_ratio, file=file.path(outdir, paste0("Cancer_ratio_Insert_size.rds")))
saveRDS(Normal_median, file=file.path(outdir, paste0("Normal_median_Insert_size.rds")))
saveRDS(zscores_mat_insertsize, file = file.path(outdir, paste0("Zscore_mat_Insert_size.rds")))
saveRDS(Insert_df_zscores, file = "Zscores df for insert size.rds")


# Check for missing samples
# Identify values in metadata_df$sample_id that are not in Insert_df_zscores$Sample
unique_sample_ids <- setdiff(metadata_df$sample_id, Insert_df_zscores$Sample)

# Print the unique sample IDs
print(unique_sample_ids)

## Join matrices together 
insert_matrix_df <- bind_rows(healthy_insert_matrix, cancer_insert_matrix)


## Make a heatmap of the insert size matrix 
insert_matrix <-as.matrix(insert_matrix_df)

# getting IDs
sample_order <- rownames(insert_matrix)
...
sample_order <- rownames(insert_matrix)
column_order <- colnames(insert_matrix)

## Add some labels for nucleotide peaks

# Initialize a vector to hold the labels
nucleotide_regions <- rep(NA, ncol(insert_matrix))

# Convert column names to numeric
column_nums <- as.numeric(colnames(insert_matrix))
...
# Ensure order of insert_matrix is correct 
insert_matrix <- insert_matrix[, column_order]

## Generate heatmap
Heatmap_insert <- Heatmap(insert_matrix,
                   col = col_heat,
...
png(filename = "Heatmap of insert size by cancer type, updated legends, updated colors, with breaks, May 2024.png", width = 30, height = 25, units = "in", res = 350)
...
zscores_ordered <- zscores_mat[match(sample_order, rownames(zscores_mat)), ]
...
Heatmap_insert2 <- Heatmap(insert_matrix,
                   col = col_heat,
                   show_row_names = FALSE,
                   show_column_names = FALSE,
                   #heatmap_legend_param = heatmap_legend_param,
...
png(filename = "Heatmap of insert size by cancer type, updated legends, updated colors, with breaks, split, May 2024.png", width = 30, height = 25, units = "in", res = 350)
...
numeric_colnames <- as.numeric(colnames(insert_matrix))

# Subset the matrix based on the numeric column names
insert_matrix_subset <- insert_matrix[, numeric_colnames >= 10 & numeric_colnames < 350]

# Initialize a vector to hold the labels
nucleotide_regions_150bp <- rep(NA, ncol(insert_matrix_subset))

# Convert column names to numeric
column_nums <- as.numeric(colnames(insert_matrix_subset))
...
## Generate heatmap
column_order_narrow <- colnames(insert_matrix_subset)

# Define a new sequential color scale
col_heat_seq <- colorRamp2(c(0, 0.007, 0.025), 
                           c("#dcedf6", "#92b4e8", "#1f78b4"))

Heatmap_insert_narrow <- Heatmap(insert_matrix_subset,
                   col = col_heat_seq,
                   show_row_names = FALSE,
                   show_column_names = FALSE,
                   #heatmap_legend_param = heatmap_legend_param,
...
png(filename = "Heatmap of insert size by cancer type, updated legends, updated colors, with breaks, May 2024, narrow, 2.png", width = 18, height = 25, units = "in", res = 350)
draw(Heatmap_insert_narrow, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()


png(filename = "Heatmap of insert size by cancer type, updated legends, updated colors, with breaks, May 2024, narrow, 3.png", width = 18, height = 23, units = "in", res = 350)
draw(Heatmap_insert_narrow, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()

png(filename = "Heatmap of insert size by cancer type, updated legends, updated colors, with breaks, May 2024, narrow, 4.png", width = 15, height = 20, units = "in", res = 350)
draw(Heatmap_insert_narrow, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()

png(filename = "Heatmap of insert size by cancer type, updated legends, updated colors, with breaks, May 2024, narrow, 5.png", width = 13, height = 17, units = "in", res = 500)
draw(Heatmap_insert_narrow, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()
...
short <- rownames_to_column(insert_matrix_df, "Sample")
...
prop_short_df <- left_join(prop, metadata_df, by = c("Sample" = "sample_id"))

## Add column 
prop_short_df$Twentyto150_pct <- prop_short_df$prop*100

## Save
write.table(prop_short_df, "Prop 20 to 150 bp in cfMeDip.txt", row.names = FALSE, sep = "\t")
saveRDS(prop_short_df, file = "Prop 20 to 150 bp in cfMeDip.rds")



## Plot 
data_stats_cancer_timepoint_prop_insert <-  prop_short_df %>%
...
ordered_cancer_types <- c("Healthy", ordered_cancer_types, "LFS_positive", "LFS_survivor", "LFS_previvor")

## Fixing head and neck naming 
ordered_cancer_types_peak <- c("Healthy", "AML", "Lung_cancer", 
...
prop_short_df$cancer_type_title_case <- gsub("Head_and_neck_cancer", "Head_and\nneck_cancer", prop_short_df$cancer_type_title_case)
prop_short_df$cancer_type_ordered <- factor(prop_short_df$cancer_type_title_case, levels = ordered_cancer_types_peak)
...
## Plot from here
png(filename = "Proportion of short reads in cfMos, May 2024, reordered, updated.png", width = 13, height = 5, units = "in", res = 500)
prop_short_df %>%
  ...
  geom_hline(yintercept = 0.1487008, linetype = "dashed", color = "black") + # added intercept from table
  ...
  scale_fill_manual(values = col_cancer_type, name = "Cancer type") + # Add name for legend title
...
#levels(prop_short_df$cancer_type_ordered) <- str_wrap(levels(prop_short_df$cancer_type_ordered), width = 10) # Adjust 'width' as needed

png(filename = "Proportion of short reads in cfMos, May 2024, reordered, bigger text, 2.png", width = 13, height = 4.5, units = "in", res = 500)
ggplot(prop_short_df, aes(x = cancer_type_ordered, y = Twentyto150_pct)) +
  ...
  scale_fill_manual(values = col_cancer_type, name = "Cancer type") +
  theme(panel.grid.major = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))
dev.off()
...
png(filename = "Proportion of short reads in cfMos, May 2024, reordered, bigger text, 5.png", width = 13, height = 4.5, units = "in", res = 500)
ggplot(prop_short_df, aes(x = cancer_type_ordered, y = Twentyto150_pct)) +
  geom_boxplot(aes(fill=cancer_type_title_case), outlier.shape = NA) +
  geom_jitter(aes(x = cancer_type_ordered, y = Twentyto150_pct), width=0.3, size = 0.4, alpha = 0.5) +
  geom_hline(yintercept = 0.1487008, linetype = "dashed", color = "black") + 
...
saveRDS(insert_matrix_df, file = "Prop_insert_sizes_updated_Dec2024.rds")
write.table(insert_matrix_df, file = "Prop_insert_sizes_updated_Dec2024.txt", sep = "\t", quote = F, row.names = T)
...
rownames(pca_df) <- rownames(insert_matrix_df)
...
ggsave(filename = file.path(outdir, "PCA_Variance_Explained_Insert_sizes.png"), plot = variance_plot, width = 10, height = 6, units = "in", dpi = 300)

# Save the PCA results to a file 
write.csv(pca_df, file.path(outdir, "PCA_results_Insert_sizes.csv"), row.names = TRUE)
saveRDS(pca_df, file = "Insert_sizes_updated_June2024_PCAs_97_percent_variance.rds")

## Export the metadata df 
write.table(metadata_df, file = "Metadata df June 2024 annotated.txt", sep = "\t", quote = F, row.names = F)
saveRDS(metadata_df, file = "Metadata df June 2024 annotated.rds")
saveRDS(pca_result_insertsize, file = "PCA result for insert size.rds")
...
# Export as .txt
write.table(insert_matrix_df, file = "Insert_matrix_df_for_Althaf_updated.txt", sep = "\t", row.names = TRUE, col.names = NA)

# Export as .rds
saveRDS(insert_matrix_df, file = "insert_matrix_df_for_Althaf_updated.rds")

## Export for the supplemental tables
write.table(insert_matrix_df, file = "Insert_matrix_dataframe.txt", sep = "\t", row.names = TRUE, quote = F)
...
leveneTest(prop ~ cancer_type_corrected_updated, data = prop_short_df)

#difference between groups 
anova_result <- aov(prop ~ cancer_type_corrected_updated, data = prop_short_df)
summary(anova_result)

kruskal_result_prop <- kruskal.test(prop ~ cancer_type_corrected_updated, data = prop_short_df)
...
pairwise_results <- pairwise.t.test(x = prop_short_df$prop,
                                    g = prop_short_df$cancer_type_corrected_updated,
                                    p.adjust.method = "holm",
                                    paired = FALSE,
                                    pool.sd = !is.factor(prop_short_df$cancer_type_corrected_updated))
...
dunn_result <- dunn.test(x = prop_short_df$prop, 
                         g = prop_short_df$cancer_type_corrected_updated, 
...
write.csv(normal_comparisons_sorted, "normal_comparisons_dunn_test_proportion_short_frags_June2024.csv", row.names = FALSE)



## Get direction
# Calculate median or mean for each group
group_medians <- prop_short_df %>%
  group_by(cancer_type_corrected_updated) %>%
...
write.table(p_values_matrix, "Pairwise results for the proportion of short fragments June 2024.txt", quote = F, sep = "\t")

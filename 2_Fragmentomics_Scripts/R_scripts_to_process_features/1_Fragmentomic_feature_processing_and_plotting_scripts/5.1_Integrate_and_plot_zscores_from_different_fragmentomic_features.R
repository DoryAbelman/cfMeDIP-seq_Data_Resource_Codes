...
#
# Inputs  :
#   • End motif genome‑wide Z‑scores
#   • Insert size genome‑wide Z‑scores
#   • DELFI genome‑wide Z‑scores
#   • Nucleosome peak genome‑wide Z‑scores
#   • Proportion of short fragments (20–150 bp)
#   • Fragmentation scores (NMF)
#   • Sample metadata
#
# Purpose :
#   1. Load each per‑sample Z‑score dataframe and the short‑fragment & NMF scores  
#   2. Rename and join them into a single wide table keyed by sequencing_id  
#   3. Merge with sample metadata (cancer type, cohort, age, etc.)  
#   4. Build a matrix of key metrics for heatmap visualization  
#   5. Generate a split‑row heatmap of all metrics per sample, grouped by cancer subtype  
#   6. Compute proportions of samples exceeding Z‑score cutoff for each metric & cancer type  
#   7. Perform spearman correlations, visualize pairwise relationships, and export summary tables  
#   8. Export all intermediate tables and figures for publication and downstream ML  
#
# ----------------------------------------------------------------------------
...
# Save
saveRDS(Merged_zscore_df, file = "Merged zscore df Dec 2024.rds")
...
png(filename = "Heatmap of frag metrics all together, Dec 2024, narrow 3.5.png", width = 13, height = 15, units = "in", res = 350)
...
subset_df_renamed <- subset_df %>%
  rename(
    ...
    Fragment_ratios_zscore = Delfi_zscore
  )

# ==============================================================================
# Purpose: 5 1A Integrate and plot zscores from different fragmentomic features
...
# How to run: Rscript 2_Fragmentomics_Scripts/R_scripts_to_process_features/2_Fragmentomic_feature_processing_and_plotting_scripts_validation_cohort/5.1A_Integrate_and_plot_zscores_from_different_fragmentomic_features.R
...
data_dir <- "Final Data Dec 2024/Zscores/Without validation/"
...
metadata_df <- readRDS("Metadata df Dec 2024 all samples.rds")
...
Fragment_scores <- readRDS(file.path(data_dir, "All_samples_FS.rds"))
...
# Save
saveRDS(Merged_zscore_df, file = "Merged zscore df March 2025.rds")
...
Heatmap_altogether_narrow_May2024 <- Heatmap(matrix_heatmap_fig,
                ...
                #              top_annotation = top_annotation,
                              ...
                              heatmap_legend_param = list(title = "Zscore to normal", 
                                                          ...
                                                          at = seq(-2, 30, by = 20))) # can remove the at and seq for other breaks
...
png(filename = "Heatmap of frag metrics all together, March 2025, narrow 3.5.png", width = 13, height = 15, units = "in", res = 350)
draw(Heatmap_altogether_narrow_May2024, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()


### Re-export
Heatmap_altogether_narrow_May2024 <- Heatmap(
  matrix_heatmap_fig,
  col = col_heat_together,
  show_row_names = FALSE,
  show_column_names = FALSE,
  right_annotation = right_annotation,
  # left_annotation removed
  row_labels = NULL,
  column_title_gp = gpar(fontsize = 12),
  row_title_gp = gpar(fontsize = 14),
  row_split = row_split,
...
png(filename = "Heatmap_no_left_annotation_merged_features_updated.png", width = 13, height = 15, units = "in", res = 500)
draw(Heatmap_altogether_narrow_May2024, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()
...
ggsave(filename = "Correlation_matrix_all_features_Dec2024.pdf", plot = corr_plot, width = 12, height = 12, units = "in", dpi = 500)
...
ggsave(filename = "Correlation_matrix_all_features_colored_Dec2024.pdf", plot = corr_plot_colored, width = 14, height = 12, units = "in", dpi = 500)
...
ggsave(filename = "Correlation_matrix_all_features_colored_manual 3 Dec 2024.pdf", plot = corr_plot_colored_manual2, width = 14, height = 12, units = "in", dpi = 500)
...
ggsave(filename = "Correlation_matrix_all_features_colored_manual 4 Dec 2024.pdf", plot = corr_plot_colored_manual3, width = 14, height = 12, units = "in", dpi = 500)
...
# Export the z-score table as a supplementary table with sequencing_id
Z_score_table_export_v2 <- Merged_zscore_df[, c("sequencing_id", "Endmotif_zscore", "Insertsize_zscore", "Delfi_zscore", 
                                                "Nucleosome_peak_zscore", "Twentyto150_pct", "FS", 
                                                "cancer_type_title_case", "sample_id")]

Z_score_table_export_v2 <- Z_score_table_export_v2 %>%
  dplyr::mutate(cancer_type_title_case = ifelse(cancer_type_title_case == "Eye_cancer", "Uveal_melanoma", cancer_type_title_case)) %>%
  rename(
    Prop_short_frags = Twentyto150_pct,
    Fragment_size_score = FS
...
# Export
write.table(Z_score_table_export_v2, sep = "\t", quote = FALSE, row.names = FALSE, 
            file = "Genome-wide Z-score table per feature with sequencing ID.txt")
...
ggsave(filename = "Correlation_matrix_all_features_colored_manual 5, naming corrected, Dec 2024.pdf", plot = corr_plot_colored_manual4, width = 14, height = 12, units = "in", dpi = 500)



### Now get the correlations in a table
# Function to calculate correlations
calculate_correlations2 <- function(data, columns, group_col) {
  ...
  for (i in seq_along(columns)) {
    for (j in seq_along(columns)) {
      ...
      data_grouped <- data %>%
        ...
        bind_rows(data %>% dplyr::mutate(group = "Overall Corr")) %>%
        group_by(group) %>%
        filter(sum(!is.na(.data[[x_col]])) > 1 & sum(!is.na(.data[[y_col]])) > 1) %>%
        summarize(
          x = x_col,
          y = y_col,
          estimate = round(as.numeric(cor.test(.data[[x_col]], .data[[y_col]], method = "spearman")$estimate), 2),
          pvalue = cor.test(.data[[x_col]], .data[[y_col]], method = "spearman")$p.value,
          pvalue_star = as.character(symnum(
            pvalue, corr = FALSE, na = FALSE,
            cutpoints = c(0, 0.001, 0.01, 0.05, 0.1, 1),
            symbols = c("***", "**", "*", "'", " ")
          ))
        ) %>%
      ...
      correlation_df_list[[paste(x_col, y_col, sep = "_")]] <- data_grouped
    }
  }
  
  # Combine all results into a single dataframe
  correlation_df <- bind_rows(correlation_df_list)
  return(correlation_df)
}

# Columns to analyze
columns_to_analyze <- colnames(subset_df_renamed)[1:6]  # Adjust this if needed
group_column <- "cancer_type_title_case"

# Generate the correlation dataframe
correlation_results_df <- calculate_correlations2(subset_df_renamed, columns_to_analyze, group_column)

# Now everything combined as a table
# Function to calculate correlations with and without grouping
calculate_correlations_combined <- function(data, columns, group_col) {
  correlation_df_list <- list()
  ...
  for (i in seq_along(columns)) {
    for (j in seq_along(columns)) {
      ...
      # Calculate correlations grouped by cancer type
      data_grouped <- data %>%
        dplyr::mutate(group = .data[[group_col]]) %>%
        bind_rows(data %>% dplyr::mutate(group = "Overall Corr")) %>%
        group_by(group) %>%
      ...
      # Calculate overall correlation without grouping
      overall_corr <- data %>%
        filter(sum(!is.na(.data[[x_col]])) > 1 & sum(!is.na(.data[[y_col]])) > 1) %>%
        summarize(
          group = "All Cancer Types",
          x = x_col,
          y = y_col,
          estimate = round(as.numeric(cor.test(.data[[x_col]], .data[[y_col]], method = "spearman")$estimate), 2),
          pvalue = cor.test(.data[[x_col]], .data[[y_col]], method = "spearman")$p.value,
      ...
      combined_result <- bind_rows(data_grouped, overall_corr)
      correlation_df_list[[paste(x_col, y_col, sep = "_")]] <- combined_result
    }
  }
...
# Generate the correlation dataframe
correlation_results_combined <- calculate_correlations_combined(subset_df_renamed, columns_to_analyze, group_column)

# View the dataframe
correlation_results_combined
...
write.csv(normal_comparisons, "normal_comparisons_dunn_test_FS_Dec2024.csv", row.names = FALSE)

# Print to check if any rows are caught
print(normal_comparisons)


### Do same with prop short 
# Subset data for analysis
df <- Merged_zscore_df %>%
  select(cancer_type_title_case, Twentyto150_pct) %>%
  filter(!is.na(Twentyto150_pct))

# Check if data meets ANOVA assumptions
# 1. Homogeneity of variances
leveneTest(Twentyto150_pct ~ cancer_type_title_case, data = df)

# 2. Normality of residuals
anova_model <- aov(Twentyto150_pct ~ cancer_type_title_case, data = df)
qqnorm(residuals(anova_model))
qqline(residuals(anova_model)) # assumptions not met
...
# Assuming ANOVA assumptions are met
anova_results <- aov(Twentyto150_pct ~ cancer_type_title_case, data = df)
summary(anova_results)
...
# If ANOVA assumptions are not met, use Kruskal-Wallis and Dunn's test
kruskal_test <- kruskal.test(Twentyto150_pct ~ cancer_type_title_case, data = df)

# Dunn's test for post-hoc comparisons
dunn_test <- dunnTest(df$Twentyto150_pct, df$cancer_type_title_case, method="bonferroni")
...
# Exporting the results to a CSV file for further inspection
write.csv(normal_comparisons, "normal_comparisons_dunn_test_Twentyto150_pct_Dec2024.csv", row.names = FALSE)

# Print to check if any rows are caught
print(normal_comparisons)


### Now export important features 

# 1,2. Export the matrix used for the heatmap (if you want it in tabular form)
write.table(matrix_heatmap_fig, 
            file = "Matrix_Heatmap_Fig_Dec2024.txt",
            sep = "\t",
            quote = FALSE,
            row.names = TRUE,
            col.names = NA)

# 3. Export the proportions of significant z-scores (abs(z-score) > 2) per cancer type
write.csv(significant_proportions,
          file = "Significant_Proportions_Zscores_cutoff2_relative_to_healthy_controls_Dec2024.csv",
          row.names = FALSE)

# 4. Export the correlation results with group-level correlations
write.csv(correlation_results_df,
          file = "Correlation_Results_ByCancerType_Dec2024.csv",
          row.names = FALSE)

# 5. Export the combined correlation results (grouped plus overall)
write.csv(correlation_results_combined,
          file = "Correlation_Results_Combined_Dec2024.csv",
          row.names = FALSE)

# 6. Export the Spearman correlation matrix among the key numeric features
write.csv(cor_matrix,
          file = "Spearman_Correlation_Matrix_Dec2024.csv",
          row.names = TRUE)

# 7. Export the adjusted p-value matrix for correlations
write.csv(adj_p_matrix,
          file = "Adjusted_Pvalue_Matrix_Dec2024.csv",
          row.names = TRUE)

...
# Read in R objects
filedir <- '~/Documents/Thesis_work/R/M4/Projects/Fragmentation_CfMeDIP_Yong/DELFI_ratios/'

basedir <- '~/Documents/Thesis_work/R/M4/Projects/Fragmentation_CfMeDIP_Yong/DELFI_ratios/'
datasets <- "~/Documents/Thesis_work/R/M4/Projects/Fragmentation_CfMeDIP_Yong/Final Data Dec 2024/DELFI/Validation only /"
outdir <- '~/Documents/Thesis_work/R/M4/Projects/Fragmentation_CfMeDIP_Yong/Final Data Dec 2024/DELFI/Validation only /'

dir.create(outdir, recursive = TRUE, showWarnings = FALSE)

## These are loaded from the 'Sample medians updated May 2024.R' file, which should be run prior
## Load in first without the validation and then process the validation at the end 
load(file=file.path(datasets, paste0("tcge.normal.median_fr_df.hg38.validation.rda")))
load(file=file.path(datasets, paste0("tcge.cancer.median_fr_df.hg38.validation.rda")))
#load(file=file.path(datasets, paste0("tcge.normal.median_fr_df.hg38.validation.rda")))
#load(file=file.path(datasets, paste0("tcge.cancer.median_fr_df.hg38.validation.rda")))
...
# Identify the samples that are liver cancer
liver_samples <- metadata_df_validation$sample_id[metadata_df_validation$cancer_type_title_case == "Liver_cancer"]

# Subset the Cancer_ratio matrix to remove these columns
Cancer_ratio <- dcast(df.fr.tcge.cancer.validation, seqnames + arm + start + end + bin ~ sample_id, value.var="ratio_centered")
...
Cancer_ratio <- Cancer_ratio[, !(colnames(Cancer_ratio) %in% liver_samples)]

## Get ratios and calculate genome-wide zscores
Normal_ratio <- dcast(df.fr.tcge.normal.validation, seqnames + arm + start + end + bin ~ sample_id, value.var="ratio_centered") 
...
zscores_mat_delfi_validation <- as.matrix(zscore_df)
...
Delfi_df_zscore_validation <- zscore_df 
Delfi_df_zscore_validation <- rownames_to_column(Delfi_df_zscore_validation, "Sample")

# Join with metadata df
Delfi_df_zscore_validation <- left_join(Delfi_df_zscore_validation, metadata_df, by = c("Sample" = "sample_id"))

### Save objects
saveRDS(Normal_ratio, file=file.path(outdir, paste0("Normal_ratio_DELFI_validation.rds")))
saveRDS(Cancer_ratio, file=file.path(outdir, paste0("Cancer_ratio_DELFI_validation.rds")))
saveRDS(Normal_median, file=file.path(outdir, paste0("Normal_median_DELFI_validation.rds")))
saveRDS(zscores_mat_delfi_validation, file = file.path(outdir, paste0("Zscore_mat_DELFI_validation.rds")))
saveRDS(Delfi_df_zscore_validation, file = file.path(outdir, paste0("Zscore_df_DELFI_validation.rds")))

### Prepare for plotting
plot_df_validation <- cbind(Normal_ratio,Cancer_ratio)
plot_df_validation <- t(plot_df_validation)

### Skipped this part 

## Prepare to make heatmap
data_samples <- data.frame(sample_id = rownames(plot_df_validation))

# Get order 
# Put in correct order
## Arrange by z-score and keep within group 
tmp <- tibble(sample_id = Delfi_df_zscore_validation$Sample, all_zscores = Delfi_df_zscore_validation$all_zscores)
...
# Apply the same ordering to your plot_df_validation and any annotation data frames
plot_df_validation <- plot_df_validation[sample_order, ]  

# Set cancer types plotting
cancer_types <- unique(metadata_df$cancer_type_corrected_updated)
...
data_cancer_type <- factor(data_cancer_type, 
                           levels = c("Healthy", "Breast_cancer", "Head_and_neck_cancer", "Melanoma", "Mixed_cancer", "Ovarian_cancer"),
                           labels = c("Healthy", "Breast_cancer", "Head_and_neck_cancer", "Melanoma", "Mixed_cancer", "Ovarian_cancer"))

data_cohort <- factor(data_cohort, 
                      levels = c("TCGE-CFMe-INSPIRE", "TCGE-CFMe-HCC"),
                      labels = c("TCGE-CFMe-INSPIRE", "TCGE-CFMe-HCC"))
...
#sample_order <- rownames(plot_df_validation)
...
# Identify rows with NA in data_cancer_type
na_rows <- metadata_df[is.na(metadata_df$cancer_type_corrected_updated), ]

if (nrow(na_rows) > 0) {
  print("Rows with NA in cancer_type_corrected_updated:")
  print(na_rows)
} else {
  print("No NA values found in cancer_type_corrected_updated.")
}

# Summarize how many NAs are present
na_count <- sum(is.na(metadata_df$cancer_type_title_case))
print(paste("Number of NA values in cancer_type_title_case:", na_count))
...
zscores_mat_delfi_validation <- zscores_mat_delfi_validation[sample_order, ]

zscores_ordered <- zscores_mat_delfi_validation[match(sample_order, rownames(zscores_mat_delfi_validation)), ]

# Make sure that the row names match the sample order
# 1. Reorder the main matrix
plot_df_validation <- plot_df_validation[sample_order, , drop = FALSE]
rownames(plot_df_validation) <- sample_order

# 2. Reorder the z-score matrix
zscores_mat_delfi_validation <- zscores_mat_delfi_validation[sample_order, , drop = FALSE]
rownames(zscores_mat_delfi_validation) <- sample_order

# 3. Use the reordered z-score matrix in your anno_lines
right_annotation <- rowAnnotation(
  "Genome-Wide\nZ-score" = anno_lines(
    zscores_mat_delfi_validation,
    add_points = TRUE,
    pch = c(16, NA),
    ylim = c(-5, 15),
...
Heatmap_delfi <- ComplexHeatmap::Heatmap(plot_df_validation,
                   col = col_heat,
...
#png(filename = file.path(outdir, "Heatmap_Z-score_DELFI_updated_in_cfMedip_5Mb_updated_legend_updated_colors_Dec_2024_new_colors_ordered_split_updated_3.png"), 
#    width = 30, height = 25, units = "in", res = 350)
#draw(Heatmap, heatmap_legend_side = "right", merge_legend = TRUE)
#dev.off()

png(filename = file.path(outdir, "Heatmap_Z-score_DELFI_updated_in_cfMedip_5Mb_updated_legend_updated_colors_Dec_2024_new_colors_ordered_split_updated_3_narrow2_validation.png"), 
    width = 24, height = 26, units = "in", res = 350)
draw(Heatmap_delfi, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()

png(filename = file.path(outdir, "Heatmap_Z-score_DELFI_validation2.png"), 
    width = 24/1.25, height = 26/1.25, units = "in", res = 350)
draw(Heatmap_delfi, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()
...
### Redo without the order by z-score 
sample_order <- rownames(endmotif_matrix) # from end motif validation

sample_order <- gsub("_motifs", "", sample_order)
...
png(filename = file.path(outdir, "Heatmap_Z-score_DELFI_updated_in_cfMedip_5Mb_updated_legend_updated_colors_Dec_2024_new_colors_ordered_split_updated_3_narrow2_validation_no_order_2.png"), 
    width = 24, height = 26, units = "in", res = 350)
draw(Heatmap_delfi, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()

png(filename = file.path(outdir, "Heatmap_Z-score_DELFI_validation2_no_order.png"), 
    width = 26/1.25, height = 26/1.25, units = "in", res = 350)
draw(Heatmap_delfi, heatmap_legend_side = "right", merge_legend = TRUE)
dev.off()
...
# Prepare data
data_df <- as.data.frame(plot_df_validation)
...
saveRDS(plot_df_validation, file = "DELFI_Ratios_updated_Dec2024_validation_cohort.rds")
write.table(plot_df_validation, file = "DELFI_Ratios_updated_Dec2024_validation_cohort.txt", sep = "\t", quote = F, row.names = T)


## Export the PCAs on the dataframe 

# Perform PCA
pca_result <- prcomp(plot_df_validation, center = TRUE, scale. = TRUE)
...
rownames(pca_df) <- rownames(plot_df_validation)
...
ggsave(filename = file.path(outdir, "PCA_Variance_Explained_Delfi_ratios_validation_cohort.png"), plot = variance_plot, width = 10, height = 6, units = "in", dpi = 300)

# Save the PCA results to a file 
write.csv(pca_df, file.path(outdir, "PCA_results_Delfi_ratios_validation_cohort.csv"), row.names = TRUE)
saveRDS(pca_df, file = "DELFI_Ratios_updated_Dec2024_PCAs_90_percent_variance_validation_cohort.rds")
saveRDS(pca_result, file = "PCA_result_Delfi_Dec2024_validation_cohort.rds")

## Export the metadata df 
write.table(metadata_df, file = "Metadata df Dec 2024 annotated for DELFI ratios.txt", sep = "\t", quote = F, row.names = F)

## Export the table 
write.table(plot_df_validation, file = "Fragment_proportions_matrix_dataframe_validation_cohort.txt", sep = "\t", row.names = TRUE, quote = F)

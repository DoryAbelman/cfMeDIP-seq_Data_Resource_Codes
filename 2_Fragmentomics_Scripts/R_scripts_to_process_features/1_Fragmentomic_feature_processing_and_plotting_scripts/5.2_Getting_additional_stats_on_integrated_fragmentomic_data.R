...
#
# Purpose :
#   • After generating the unified Z‑score dataframe (Merged_zscore_df), compute
#     detailed summary statistics (mean, min, max, median, SD) for each feature
#     (Endmotif, Insert size, DELFI, Nucleosome peak) by cancer subtype.
#   • Calculate per‑feature and overall metrics both within and across groups.
#   • Assess variance homogeneity via Levene’s test and highlight differences
#     between healthy controls and cancer cohorts.
#   • Identify which fragmentation metric most strongly distinguishes each cancer
#     subtype (highest mean Z‑score).
#   • Export all tables for downstream reporting and supplemental materials.
#
# ----------------------------------------------------------------------------
...
### Run this AFTER the other 5.1 script 

## First need to load the z-score DFs together and then merge them 
...
### This is what need to get z-score per cancer type
...
## Now get means 
## This is the mean of each feature across all cohorts 
# Calculate the mean of each numeric column in feature summary
column_means <- feature_summary %>%
...
## See which mean highest for which feature 
# Find the highest mean for each cancer type
### Find which cancer type had the highest overall z-score features 
...
### This is which feature was most important in distinguishing each cancer type
...
# Save summary table with detailed statistics for each z-score by cancer type
write.table(summary_table, file = "Detailed_Summary_Statistics_Per_Zscore_And_CancerType.txt", sep = "\t", quote = FALSE, row.names = FALSE)

# Save overall summary table including overall and per cancer type statistics
write.table(summary_table_2, file = "Combined_Overall_And_PerCancerType_Zscore_Statistics.txt", sep = "\t", quote = FALSE, row.names = FALSE)

# Save feature-specific summary statistics by cancer type
write.table(feature_summary, file = "Feature_Specific_Statistics_Per_CancerType.txt", sep = "\t", quote = FALSE, row.names = FALSE)

# Save overall statistics for all cancer types
write.table(overall_summary, file = "Overall_Zscore_Statistics_Per_CancerType.txt", sep = "\t", quote = FALSE, row.names = FALSE)

# Save mean z-scores for each feature across all cancer types (excluding healthy samples)
write.table(mean_zscores, file = "Mean_Zscores_By_Feature_Excluding_Healthy.txt", sep = "\t", quote = FALSE, row.names = FALSE)

# Save standard deviation statistics for each feature and overall by cancer type
write.table(sd_table, file = "Standard_Deviation_Per_Feature_And_CancerType.txt", sep = "\t", quote = FALSE, row.names = FALSE)

# Save average values of feature-specific summary statistics across all cancer types
write.table(column_means, file = "Average_Feature_Specific_Statistics_Across_CancerTypes.txt", sep = "\t", quote = FALSE, row.names = FALSE)

# Save summary report comparing healthy samples vs cancer types with standard deviations and variance analysis
write.table(summary_report, file = "Comparison_Healthy_Vs_Cancer_Standard_Deviation_Analysis.txt", sep = "\t", quote = FALSE, row.names = FALSE)

# Save table identifying the feature with the highest mean z-score for each cancer type
write.table(result, file = "Top_Feature_By_Highest_Mean_Zscore_Per_CancerType.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
## Summarize mean, range, median, and standard deviation of z-scores for each cancer type and method
# Identify the top 2 features (methods) for each cancer type based on the highest mean z-score
top_2_features <- summary_stats %>%
  group_by(cancer_type_corrected_updated) %>%
  arrange(desc(mean_zscore)) %>%
  slice_head(n = 2) %>%
  ungroup()
